...
import { deleteSkill } from "../../../services/dashboard";
...
export default async function (req: NextApiRequest, res: NextApiResponse) {
    ...
    const { user_id, dashboard_skill_id } = req.body;

    // Error managment
    if (user_id && dashboard_skill_id) {
        ...
        try {
            // Database query
            const deleted_dashboard_skill = await deleteSkill(dashboard_skill_id)

            res.status(200);
            res.json(deleted_dashboard_skill);

        } catch (e) {
            res.status(500);
            res.json({ error: "Unable to delete dashboard_skill with dashboard_skill_id: " + dashboard_skill_id.toString() });
            console.log(e);
        }
